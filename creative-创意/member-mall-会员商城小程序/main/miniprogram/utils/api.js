...
// 更新用户头像和昵称
const updateProfile = (avatarUrl, nickname) => {
  if (USE_MOCK) return { success: true, data: { avatar_url: avatarUrl, nickname } };
  return callFunction('user', {
...
// 获取统计数据
const getStatistics = () => {
  return callFunction('admin', { action: 'getStatistics' });
};

// 获取商品列表（管理）
const getProductListAdmin = (page = 1, pageSize = 20) => {
  return callFunction('admin', { action: 'getProductList', page, page_size: pageSize });
};

// 添加商品
const addProduct = (product) => {
  return callFunction('admin', { action: 'addProduct', product });
};

// 更新商品
const updateProduct = (productId, product) => {
  return callFunction('admin', { action: 'updateProduct', product_id: productId, product });
};

// 删除商品
const deleteProduct = (productId) => {
  return callFunction('admin', { action: 'deleteProduct', product_id: productId });
};

// 获取会员列表
const getUserListAdmin = (page = 1, pageSize = 20, role = '') => {
  return callFunction('admin', { action: 'getUserList', page, page_size: pageSize, role });
};

// 获取官方邀请码列表
const getInviteCodeListAdmin = (page = 1, pageSize = 20) => {
  return callFunction('admin', { action: 'getInviteCodeList', page, page_size: pageSize });
};

// 生成官方邀请码
const generateOfficialInviteCode = (remark = '') => {
  return callFunction('admin', { action: 'generateOfficialInviteCode', remark });
};

// 删除官方邀请码
const deleteInviteCodeAdmin = (codeId) => {
  return callFunction('admin', { action: 'deleteInviteCode', code_id: codeId });
};

// 更新邀请码状态
const updateInviteCodeStatus = (codeId, status) => {
  return callFunction('admin', { action: 'updateInviteCodeStatus', code_id: codeId, status });
};

// 获取福利码列表
const getWelfareCodeListAdmin = (page = 1, pageSize = 20) => {
  return callFunction('admin', { action: 'getWelfareCodeList', page, page_size: pageSize });
};
...
module.exports = {
  ...
  deleteProduct,
  getUserListAdmin,
  getInviteCodeListAdmin,
  generateOfficialInviteCode,
  deleteInviteCodeAdmin,
  updateInviteCodeStatus,
  getWelfareCodeListAdmin
};
