...
Page({
  ...
  async loadInviteInfo() {
    ...
    if (res.success) {
      const inviteCount = res.data.invite_count || 0;
      const inviteLimit = res.data.invite_limit || 5;
      const remainingCount = Math.max(0, inviteLimit - inviteCount);
  ...
  onShareAppMessage() {
    return {
      ...
      path: `/pages/benefits/benefits?invite_code=${this.data.inviteCode}`
    };
  }
});
