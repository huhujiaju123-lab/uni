...
### 首页改版
- **精选样例展示**：新增「先看看效果」showcase 网格，6 期精选（美国打伊朗/马女士聊天/达美乐中国/贝叶斯/李笑来脱不花/妈妈桑），双列卡片 + 主题标签
- **个人历史折叠**：「已生成」改为可折叠的「我的记录」，减少首屏干扰
- 容器宽度 600→680px，移动端单列适配

### 进度页优化
- 去掉不准确的预估时间，步骤仅显示已用时间
- 底部提示改为「生成需要一些时间，请耐心等待。用电脑打开生成更方便哦」

### v2.2.1 (2026-03-01)
修复参与者幻觉问题
- analyzer.py CLI 模式自动加载 metadata.json，避免 AI 用世界知识猜测参与者
- SYSTEM_PROMPT 增加反幻觉规则：参与者姓名只能从标题/简介/转录提取

### 改动文件
- `core.py`：新增 SHOWCASE_TAGS + get_showcase()
- `web.py`：index 路由传 showcase 数据
- `templates/index.html`：showcase 网格 + 折叠历史
- `templates/progress.html`：去预估时间
