...
ARCHIVE_DIR = os.path.join(BOT_DIR, "received_files")
CSV_PREVIEW_ROWS = 3
...
def safe_filename(name: str) -> str:
    """生成适合落盘的安全文件名"""
    cleaned = re.sub(r"[^\w.\- ]+", "_", name or "unknown_file")
    cleaned = cleaned.strip().replace(" ", "_")
    return cleaned or "unknown_file"


def classify_csv(filename: str) -> str:
    """按文件名识别常见日报数据类型"""
...
def download_message_file(message_id: str, file_key: str) -> Tuple[str, str]:
    """
    下载飞书消息里的文件资源
    返回 (本地路径, 飞书返回的文件名)
    """
    ...
    if not response.success():
        raise RuntimeError(f"下载文件失败: code={response.code}, msg={response.msg}")

    date_dir = os.path.join(ARCHIVE_DIR, time.strftime("%Y-%m-%d"))
    os.makedirs(date_dir, exist_ok=True)

    file_name = safe_filename(response.file_name or f"{file_key}.bin")
    local_path = os.path.join(date_dir, f"{int(time.time())}_{file_name}")
...
def summarize_csv(local_path: str, original_name: str) -> str:
    """读取 CSV 基本信息，便于后续聚合和排错"""
    row_count = 0
    preview_rows = []
    ...
    with open(local_path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            row_count += 1
            if len(preview_rows) < CSV_PREVIEW_ROWS:
                preview_rows.append(row)

    lines = [
        f"已接收文件：{original_name}",
        f"识别类型：{classify_csv(original_name)}",
        f"字段数：{len(fieldnames)}",
        f"数据行数：{row_count}",
        f"归档路径：{os.path.basename(local_path)}",
    ]

    if fieldnames:
        lines.append(f"字段：{', '.join(fieldnames[:8])}")
        if len(fieldnames) > 8:
            lines[-1] += " ..."
    ...
    if preview_rows:
        ...
        for key in fieldnames[:4]:
            value = preview_rows[0].get(key, "")
            sample.append(f"{key}={value}")
        if sample:
            lines.append("样例首行：" + " | ".join(sample))
...
def send_text_to_chat(chat_id: str, text: str):
    ...
    if not resp.success():
        logger.error(f"[飞书] 主动发消息失败: code={resp.code}, msg={resp.msg}")
...
def handle_message(data: P2ImMessageReceiveV1):
    ...
    try:
        ...
        if msg_type == "text":
            ...
            if not file_key:
                reply_msg(message_id, "道友传来的文件缺少 file_key，韩某无法取回。")
                return

            local_path, downloaded_name = download_message_file(message_id, file_key)
            logger.info(f"[文件] 已归档 {downloaded_name} -> {local_path}")

            if downloaded_name.lower().endswith(".csv"):
                summary = summarize_csv(local_path, downloaded_name)
                reply_msg(message_id, summary)
            else:
                reply_msg(
                    message_id,
                    "\n".join([
                        f"已接收文件：{downloaded_name}",
                        f"归档路径：{os.path.basename(local_path)}",
                        "当前仅自动解析 CSV，其他类型已先保存待后续处理。",
                    ])
                )
            return
        else:
