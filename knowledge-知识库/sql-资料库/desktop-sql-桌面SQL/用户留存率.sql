...
target_day_users AS (
    SELECT 
        ...
        b.shop_name,
        -- 添加用户注册日期
        DATE(CONVERT_TZ(u.create_time, @@time_zone, 'America/New_York')) AS register_date,
        -- 添加是否当天注册的标识
        CASE 
            WHEN DATE(CONVERT_TZ(u.create_time, @@time_zone, 'America/New_York')) = 
                 DATE(CONVERT_TZ(a.create_time, @@time_zone, 'America/New_York'))
            THEN 1 ELSE 0 
        END AS is_same_day_register
    FROM ods_luckyus_sales_order.t_order_item a
    LEFT JOIN ods_luckyus_sales_order.v_order b ON a.order_id = b.id
    LEFT JOIN ods_luckyus_sales_crm.t_user u ON b.user_no = u.user_no
    ...
    AND one_category_name='Drink'
    AND INSTR(COALESCE(u.tenant, 'LKUS'), 'IQ') = 0  -- 添加用户表的租户过滤
    GROUP BY DATE(CONVERT_TZ(a.create_time, @@time_zone, 'America/New_York')), b.user_no, b.shop_name, 
             DATE(CONVERT_TZ(u.create_time, @@time_zone, 'America/New_York'))
),

-- 3. 获取这些新用户后续的交易活动（包含店铺信息）
...
daily_trade_users AS (
    SELECT 
        ...
        -- 添加当天注册用户统计
        COUNT(DISTINCT CASE WHEN is_same_day_register = 1 THEN user_no END) AS same_day_register_users,
        -- 添加当天注册占比
        ROUND(COUNT(DISTINCT CASE WHEN is_same_day_register = 1 THEN user_no END) * 100.0 / COUNT(DISTINCT user_no), 2) AS same_day_register_rate_pct,
        -- 添加平均注册间隔天数
        ROUND(AVG(DATEDIFF(trade_date, register_date)), 2) AS avg_register_to_trade_days
...
repurchase_analysis AS (
    SELECT 
        dtu.trade_date,
        dtu.shop_name,
        COUNT(DISTINCT dtu.user_no) AS total_new_users,
        ...
        ROUND(COUNT(DISTINCT d7ufp.user_no) * 100.0 / COUNT(DISTINCT dtu.user_no), 2) AS d7_users_d8_d21_vs_total_new_users_pct
...
-- 7. 最终新用户留存率分析表（包含当日留存、累计留存、店铺维度和注册信息）
SELECT 
    ...
    -- 新增：注册相关信息（只在D0当天显示）
    CASE WHEN rb.days_since_target_date = 0 THEN dtu.same_day_register_users ELSE NULL END AS same_day_register_users,
    CASE WHEN rb.days_since_target_date = 0 THEN dtu.same_day_register_rate_pct ELSE NULL END AS same_day_register_rate_pct,
    CASE WHEN rb.days_since_target_date = 0 THEN dtu.avg_register_to_trade_days ELSE NULL END AS avg_register_to_trade_days,
