...
OUTPUT_REPORTS_DIR = BASE_DIR / "outputs-输出" / "reports-报告"
OUTPUT_DATA_DIR = BASE_DIR / "outputs-输出" / "data-数据快照"
...
def generate_daily_report(dates: list):
    ...
    # 写入文件
    report_content = "\n".join(output)
    report_file = OUTPUT_REPORTS_DIR / f"daily_report_{dates[-1]}.md"
    ...
    # 保存原始数据
    if not business_df.empty:
        business_df.to_csv(OUTPUT_DATA_DIR / 'daily_business.csv', index=False)
    if not user_df.empty:
        user_df.to_csv(OUTPUT_DATA_DIR / 'daily_user.csv', index=False)
    if not category_df.empty:
        category_df.to_csv(OUTPUT_DATA_DIR / 'daily_category.csv', index=False)
    if not funnel_df.empty:
        funnel_df.to_csv(OUTPUT_DATA_DIR / 'daily_funnel.csv', index=False)
    if not product_df.empty:
        product_df.to_csv(OUTPUT_DATA_DIR / 'daily_product.csv', index=False)
